...
def analyze_regions(regions,input_wssd,mask_wssd,output_file,group):
	...
	for region in regions:
		...
		f.set_figheight(5)
		#f.set_figheight(100)
		depth = input_wssd.depth[group][region.chr][region.start:region.end,:,0].sum(1)
...
def load_region_depths(regions,wssd_input,mask_wssd,dim,input_group,indiv,edit):
	...
	for region in regions:
		...
		if(edit == -1):
			depth = wssd_input.depth[input_group][region.chr][region.start:region.end,:,dim].sum(1)
		else:
			depth = wssd_input.depth[input_group][region.chr][region.start:region.end,edit,dim]
