...
class LinearRegress:
    ...
    def fit(self,xvals_use=None,thruZero=False):
        if xvals_use==None:
            ...
            liuseX=np.array([i for i in xrange(self.x.shape[0]) if abs(self.x[i]-xvals_use).min()<1e-6 ],'int32')
...
class TwoPieceNonSimulLinearRegress:
    ...
    def fit(self,xvals_use_seg0=None,xvals_use_seg1=None,thruZero=False):
        if xvals_use_seg0==None:
            ...
            liuseX_0=np.array([i for i in xrange(self.x.shape[0]) if abs(self.x[i]-xvals_use_seg0).min()<1e-6 ],'int32')
            ...
            if liuseX_0.shape[0]==0:
                ...
                if xvals_use_seg1 == None:
                    ...
                    liuseX_1 = np.array([ i for i in xrange(self.x.shape[0]) 
                    ...
                    liuseX_1=np.array([i for i in xrange(self.x.shape[0]) if abs(self.x[i]-xvals_use_seg1).min()<1e-6 ],'int32')
                    self.xrngDomainLeftInclusive=[0., self.x[liuseX_1].min() - 1e-10 ]
...
class ml_get_cp2_trunc_gaussian:
    def __init__(self,fnfit_params,lUseCp=None,thruZero=False,max_cp=100):
        ...
        self.trunc_rescale_ll = np.array( np.log( [ 1.0 - norm.cdf(0.,self.mus[icp],self.sigs[icp]) for icp in xrange(self.cns.shape[0]) ] ) )
        self.lImpropNorms=[ norm(self.mus[icp],self.sigs[icp]) for icp in xrange(self.cns.shape[0]) ]
        
        for icp in xrange(self.cns.shape[0]):
    ...
    def get_mle_cp(self,_depth):
        ...
        LL = np.array( [ (np.log( self.lImpropNorms[icp].pdf(depth) + 1e-100 ) - self.trunc_rescale_ll[icp]).sum() for icp in xrange(self.cns.shape[0]) ], 'float64'  )
...
class ml_get_cp2_cont_poisson_twoPieceRegress:
    ...
    def get_mle_cp(self,_depth):
        ...
        LL = np.array( [ (depth*np.log(self.mus[icp])).sum() - 
                        ...
                        N*self.mus[icp] + 1e-100 for icp in xrange(self.cns.shape[0]) ], 'float64') 
